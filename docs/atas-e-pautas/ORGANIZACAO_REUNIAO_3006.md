...
```mermaid
graph TD
    A["Edital FINEP Mais Inovação<br/>R$ 5-30M | ICT obrigatório<br/>Contrapartida financeira"] --> B{"Estratégia"}

    B --> C["Caminho A (IMEDIATO)<br/>Núcleo: Painéis Michel<br/>Patente + CDHU + Caixa"]
    B --> D["Caminho B (FUTURO)<br/>Fábrica Escola<br/>Multi-tecnologias"]

    C --> E["P&D dentro do projeto<br/>Redução concreto/aço<br/>Novos agregados<br/>Materiais alternativos"]

    E --> F["Linhas de Pesquisa"]
    F --> G1["Bambu + PU Vegetal<br/>(Fabio + Imperveg)"]
    F --> G2["PU Vegetal expansivo<br/>(MAMONEX RD70)"]
    F --> G3["Resíduos agrícolas<br/>(cana, milho, palha)"]
    F --> G4["Resíduos plásticos<br/>(Eco Fabri - Sérgio Prado)"]
    F --> G5["Pó de rocha<br/>(Gisele/Embrapa - agroecologia)"]
    F --> G6["Terra + Bambu<br/>(Bahareque - Colômbia)"]
    F --> G7["Geodésicas bambu<br/>(Prof. Basta - estufas)"]
    F --> G8["Cannabis/hemp<br/>(Maurilio + Flor da Vida)"]

    C --> H["Contrapartida"]
    H --> H1["Michel + parceiros<br/>(prototipagem)"]
    H --> H2["Imperveg<br/>(material - PU Vegetal)"]

    C --> I["ICT Parceiro"]
    I --> I1["UFSCar (São Carlos)<br/>Concreto + Materiais Naturais<br/>PU Mamona (histórico)"]
    I --> I2["IPT (São Paulo)<br/>Certificação, ensaios<br/>Já tem contato"]
    I --> I3["UNESP/Inova (Campinas)"]
    I --> I4["São José dos Campos<br/>Polo de Inovação + IPT"]

    C --> J["Documentos"]
    J --> J1["Carta de Intenções<br/>Empresa ↔ ICT"]
    J --> J2["Projeto técnico<br/>(Fabio produz)"]
    J --> J3["Documentação financeira<br/>empresas parceiras"]

    K["ABNT Comissão 6263<br/>(André é membro)"] --> L["Novas normas saindo:"]
    L --> L1["Plano Diretor"]
    L --> L2["Infraestrutura Verde"]
    L --> L3["Indicadores Sustentabilidade"]
    L --> L4["Impacto de Vizinhança"]

    D --> M["Assistência Técnica<br/>Integrada (ATI)"]
    M --> M1["Habitação"]
    M --> M2["Alimentação"]
    M --> M3["Saúde Integral"]
    M --> M4["Escritórios Itinerantes"]

    subgraph "Prazo FINEP"
    N["31/08/2026<br/>Submissão"]
    end
```
